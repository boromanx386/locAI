...
class SettingsDialog(QDialog):
    ...
    def create_ollama_tab(self) -> QWidget:
        ...
        self.context_window_spin.setToolTip("Maximum context size in tokens (512-32768)")
        ...
        self.temperature_spin.setToolTip("Creativity: Higher = more creative, Lower = more focused (0.0-2.0)")
        ...
        self.repeat_penalty_spin.setToolTip("Penalty for repetition (1.0 = no penalty, 0.0-2.0)")
        ...
        self.max_history_spin.setToolTip("Maximum number of previous messages to remember (0 = unlimited)")
        ...
        self.explicit_history_check.setToolTip("Send conversation history explicitly in prompt (instead of only using context)")
    ...
    def create_image_gen_tab(self) -> QWidget:
        ...
        MaterialIcons.apply_to_button(self.refresh_models_btn, MaterialIcons.REFRESH_SVG, size=18)
...
<path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z" fill="currentColor"/>
        ...
        MaterialIcons.apply_to_button(self.model_info_btn, MaterialIcons.INFO_SVG, size=18)
        ...
        self.auto_apply_check.setToolTip("Automatically apply saved preset settings when selecting a model")
        ...
        self.negative_prompt_edit.setPlaceholderText("blurry, low quality, distorted, ugly...")
        ...
        MaterialIcons.apply_to_button(self.save_preset_btn, MaterialIcons.SAVE_SVG, size=18)
        self.save_preset_btn.setToolTip("Save current settings as preset for selected model")
        self.save_preset_btn.clicked.connect(self.save_current_preset)
        preset_buttons_layout.addWidget(self.save_preset_btn)
        
        self.reset_preset_btn = QPushButton("Reset to Preset")
        MaterialIcons.apply_to_button(self.reset_preset_btn, MaterialIcons.RESET_SVG, size=18)
        self.reset_preset_btn.setToolTip("Reset settings to saved preset for selected model")
        ...
        MaterialIcons.apply_to_button(self.delete_preset_btn, MaterialIcons.DELETE_SVG, size=18)
    ...
    def create_lora_tab(self) -> QWidget:
        ...
        MaterialIcons.apply_to_button(self.refresh_lora_btn, MaterialIcons.REFRESH_SVG, size=18)
        ...
        self.lora_weight_spin.setToolTip("LoRA strength/weight (0.0 = no effect, 1.0 = full effect, 2.0 = double effect)")
        ...
        self.lora_info_label.setStyleSheet("color: #888; padding: 8px; min-height: 60px;")
        ...
        self.lora_info_edit.setPlaceholderText("Enter information about this LoRA model...")
        ...
        MaterialIcons.apply_to_button(self.save_lora_info_btn, MaterialIcons.SAVE_SVG, size=18)
            ...
            try:
                ...
                for lora in models.get("loras", []):
                    if lora["display"] == lora_name or lora["name"] == lora_name:
                        ...
                        self._current_lora_identifier = lora["path"]  # Use path as unique identifier
                        ...
                        if saved_info:
                            info_text += f"<br><b>Saved Information:</b><br>{saved_info}"
                        elif "info" in lora and lora["info"]:
                            info_text += f"<br><b>Information:</b><br>{lora['info']}"
                        else:
                            info_text += "<br><i>No additional information available.</i>"
                ...
                self.lora_info_label.setText(f"Error loading LoRA information: {str(e)}")
                self.lora_info_edit.setPlainText("")
                self.lora_info_edit.setEnabled(False)
                self.save_lora_info_btn.setEnabled(False)
                self._current_lora_identifier = None
    ...
    def save_lora_info(self):
        ...
        if not hasattr(self, '_current_lora_identifier') or not self._current_lora_identifier:
            QMessageBox.warning(self, "No LoRA Selected", "Please select a LoRA model first.")
            return
        ...
        QMessageBox.information(self, "Information Saved", f"Information for LoRA model has been saved.")
    ...
    def show_model_info(self):
        ...
        # Info URL
        url_edit = QLineEdit()
        url_edit.setPlaceholderText("https://huggingface.co/... or https://civitai.com/...")
        if preset:
        ...
        url_edit.textChanged.connect(lambda: open_url_btn.setEnabled(bool(url_edit.text())))
        ...
        preset_width_label = QLabel(str(preset.get("width", "N/A")) if preset else "N/A")
        preset_layout.addRow("Width:", preset_width_label)
        
        preset_height_label = QLabel(str(preset.get("height", "N/A")) if preset else "N/A")
        preset_layout.addRow("Height:", preset_height_label)
        
        preset_steps_label = QLabel(str(preset.get("steps", "N/A")) if preset else "N/A")
        preset_layout.addRow("Steps:", preset_steps_label)
        
        preset_guidance_label = QLabel(str(preset.get("guidance_scale", "N/A")) if preset else "N/A")
        ...
        save_btn.clicked.connect(lambda: self.save_model_info(dialog, model_id, name_edit.text(), desc_edit.toPlainText(), url_edit.text()))
    ...
    def save_model_info(self, dialog, model_id, name, description, info_url):
        ...
        preset["name"] = name if name else model_id.split("/")[-1] if "/" in model_id else model_id
    ...
    def update_preset_info(self):
        ...
        if preset:
            ...
            self.preset_info_label.setText(f"No preset saved for <b>{model_id}</b><br><small>Configure settings and click 'Save Current Settings as Preset' to create one.</small>")
            self.reset_preset_btn.setEnabled(False)
            self.delete_preset_btn.setEnabled(False)
    ...
    def save_current_preset(self):
        ...
        preset = {
            ...
            "info_url": "",
            "description": "",
            "type": "xl" if "xl" in model_id.lower() or self.image_width_spin.value() >= 1024 else "base"
        }
    ...
    def load_settings(self):
        ...
        # Ollama
        ollama_url = self.config_manager.get("ollama.base_url", "http://localhost:11434")
        ...
        system_prompt = self.config_manager.get("ollama.conversation.system_prompt", "You are a helpful AI assistant.")
        self.system_prompt_edit.setPlainText(system_prompt)
        
        max_history = self.config_manager.get("ollama.conversation.max_history_messages", 20)
        self.max_history_spin.setValue(max_history)
        
        use_explicit = self.config_manager.get("ollama.conversation.use_explicit_history", False)
        ...
        image_model = self.config_manager.get("image_gen.model", "stabilityai/stable-diffusion-xl-base-1.0")
    ...
    def save_settings(self):
        ...
        # LLM Parameters
        self.config_manager.set("ollama.llm_params.num_ctx", self.context_window_spin.value())
        self.config_manager.set("ollama.llm_params.temperature", self.temperature_spin.value())
        ...
        self.config_manager.set("ollama.llm_params.repeat_penalty", self.repeat_penalty_spin.value())
        self.config_manager.set("ollama.llm_params.num_predict", self.max_tokens_spin.value())
        
        # Conversation Settings
        self.config_manager.set("ollama.conversation.system_prompt", self.system_prompt_edit.toPlainText())
        self.config_manager.set("ollama.conversation.max_history_messages", self.max_history_spin.value())
        self.config_manager.set("ollama.conversation.use_explicit_history", self.explicit_history_check.isChecked())
        ...
        self.config_manager.set("models.auto_download", self.auto_download_check.isChecked())
        ...
        # Image Generation
        self.config_manager.set("image_gen.enabled", self.image_enabled_check.isChecked())
        ...
        self.config_manager.set("image_gen.negative_prompt", self.negative_prompt_edit.toPlainText())
